...
<!--          &lt;!&ndash;          <div class="scroll-ele">&ndash;&gt;-->
  ...
  <div class="wraps">
    <div class="wrapper" @click.prevent="playVideo()">
      ...
      <div class="wrap" v-show="wrapShow">
        <div class="wrapperLeft" v-show="currentTimeShow">
        ...
        <div class="wrapperRight" v-show="fullscreen">
          <i class="iconfont iconliuyan" ></i>
        </div>
...
export default {
      ...
      data () {
        return {
          ...
          wrapShow: true, // 遮罩层显示
          currentTimeShow: false, // 显示播放时间进度
          controlBtn: false, // 进度按钮
          fullscreen: false, // 是否全屏
          plays: false, // 播放或暂停按钮显示出来
          play: true, // 播放和暂停按钮切换
      ...
      methods: {
        ...
        getvVideoUrl () {
          this.$api.video.videourl(this.currentVid).then(res => {
            this.videourls = res.data.urls[0].url
            console.log(this.videourl)
          })

                // this.$api.video.videolist(this.currentVid).then((res) => {
                ...
                //     this.videourl =  this.videos[0].data.urlInfo.url;
                // })
        },
            // 展开还是隐藏视频描述等
        toggle () {
